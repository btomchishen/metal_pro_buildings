...
class AviviLeadSumStatistics extends LeadDataSource
{
	...
	*/
	...
	public function getList(array $params)
	{
        ...
        $default = true; // Avivi #20215 Reports Configuration
//        fp($params, 'aParams', true);
        $config_filter = [];
        if ($semanticID === 'P') {
            $config_filter = \ReportAnalyticsConfig::get_filter('lead', 'LEAD_IN_PROGRESS');
        } else if ($semanticID === 'S') {
            $config_filter = \ReportAnalyticsConfig::get_filter('lead', 'LEAD_CONVERTED');
        } else if ($semanticID === 'F') {
            $config_filter = \ReportAnalyticsConfig::get_filter('lead', 'LEAD_FAILED');
        }
        ...
        if ($default) {
            ...
            $query->addFilter('=STATUS_ID', $config_filter);
