...
class AviviManagerCounters extends DataSource // Avivi #20215 Reports Configuration
{
...
//		$result = array_slice($result, 0, 5); // Avivi #20215 Reports Configuration
        $structure = \CIntranetUtils::getStructure();
        $sales_department_id = 3;
        $sales_employees_ids = $structure['DATA'][$sales_department_id]['EMPLOYEES'];
        foreach ($result as $key => $user) {
            if (!in_array($user['USER_ID'], $sales_employees_ids)) {
                unset($result[$key]);
            }
        }
